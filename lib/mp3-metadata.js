...
function summarizeId3Frame(frame) {
  ...
  if (frame.id === "WXXX") {
    const parts = splitTextFrameParts(frame.data);

    return {
      id: frame.id,
      label: parts[0] ? `Custom URL: ${parts[0]}` : "Custom URL",
      value: truncateFrameValue(parts.slice(1).join(" "))
    };
  }
  ...
  if (/^W[A-Z0-9]{3}$/.test(frame.id)) {
    return {
      id: frame.id,
      label: frame.id,
      value: truncateFrameValue(frame.data.toString("latin1"))
    };
  }
