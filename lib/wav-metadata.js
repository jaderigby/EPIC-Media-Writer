...
const { records: readRiffChunkRecords, assertUnrelatedChunksPreserved, writeVerified } = require("../vendor/audio-meta-armor/wav");
...
function decodeInfoSubchunkValue(data) {
  return Buffer.from(data || Buffer.alloc(0))
    .toString("utf8")
    .replace(/\0+$/, "");
}

function getListTypeFromChunkRecord(record) {
  if (!record || record.id !== "LIST" || record.bytes.length < 12) {
    return "";
  }

  return record.bytes.toString("ascii", 8, 12);
}

function readListChildRecords(record) {
  const children = [];
  ...
  let offset = 12;
  const end = 8 + record.size;

  while (offset + 8 <= end) {
    const id = record.bytes.toString("ascii", offset, offset + 4);
    const size = record.bytes.readUInt32LE(offset + 4);
    const dataStart = offset + 8;
    const dataEnd = dataStart + size;
    const childEnd = dataEnd + (size % 2);

    if (dataEnd > end || childEnd > end) break;

    children.push({
      id,
      size,
      value: isMostlyText(record.bytes.subarray(dataStart, dataEnd))
        ? truncateMetadataValue(record.bytes.toString("utf8", dataStart, dataEnd))
        : `${size} bytes`
    });
...
function buildListInfoAdditionalData(record) {
  const standardInfoIds = new Set([
    ...Object.values(WAV_INFO_MAP),
    "ISFT"
  ]);
  const subchunks = readInfoSubchunks(record.bytes, 8, record.size)
    .filter(subchunk => !standardInfoIds.has(subchunk.id));

  if (!subchunks.length) return null;

  return makeAdditionalDataItem({
    id: `wav:list-info:${record.dataStart - 8}`,
    label: "LIST/INFO",
    value: `${subchunks.length} additional field${subchunks.length === 1 ? "" : "s"}`,
    offset: record.dataStart - 8,
    size: record.size,
    children: subchunks.map(subchunk => makeAdditionalDataItem({
      id: `wav:list-info:${record.dataStart - 8}:${subchunk.id}`,
      label: subchunk.id,
      value: truncateMetadataValue(decodeInfoSubchunkValue(subchunk.data)),
      size: subchunk.data.length
    }))
  });
}

function buildWavAdditionalDataItem(buffer, record) {
  const offset = record.dataStart - 8;
  const dataStart = record.dataStart;
  const dataEnd = dataStart + record.size;
  const data = buffer.subarray(dataStart, dataEnd);
  const trimmedId = record.id.trim() || record.id;

  if (record.id === "fmt " || record.id === "data") return null;
  if (record.id === EPIC_CHUNK_ID || record.id === ARTW_CHUNK_ID) return null;

  if (record.id === "LIST") {
    const listType = getListTypeFromChunkRecord(record);

    if (listType === "INFO") {
      return buildListInfoAdditionalData(record);
    }

    const childRecords = readListChildRecords(record);

    return makeAdditionalDataItem({
      id: `wav:list:${offset}`,
      label: `LIST/${listType || "unknown"}`,
      value: childRecords.length
        ? `${childRecords.length} child item${childRecords.length === 1 ? "" : "s"}`
        : `${record.size} bytes`,
      offset,
      size: record.size,
      children: childRecords.map(child => makeAdditionalDataItem({
        id: `wav:list:${offset}:${child.id}`,
        label: child.id,
        value: child.value,
        size: child.size
      }))
  ...
  if (record.id === "iXML" || record.id === "XMP ") {
    return makeAdditionalDataItem({
      id: `wav:${trimmedId}:${offset}`,
      label: trimmedId,
      value: truncateMetadataValue(data.toString("utf8")),
      offset,
      size: record.size
  ...
  if (record.id === "ID3 " || record.id === "id3 ") {
    return makeAdditionalDataItem({
      id: `wav:id3:${offset}`,
      label: "ID3-in-WAV",
  ...
  return makeAdditionalDataItem({
    id: `wav:chunk:${trimmedId}:${offset}`,
    label: trimmedId,
    value: isMostlyText(data)
      ? truncateMetadataValue(data.toString("utf8"))
      : `${record.size} bytes`,
    offset,
    size: record.size
  });
...
function buildWavAdditionalData(buffer) {
  return readRiffChunkRecords(buffer)
    .map(record => buildWavAdditionalDataItem(buffer, record))
    .filter(Boolean);
}
...
async function readWavMetadata({ path }) {
    ...
    if (id === EPIC_CHUNK_ID) {
      ...
      try {
        ...
        epicMetadata =
          ...
          Object.prototype.hasOwnProperty.call(parsed, "epicx")
            ? parsed
            : { epicx: raw };
      } catch (err) {
        epicMetadata = {
  ...
  return {
    ...
    listInfoChunks,
    wavChunks,
    additionalData: buildWavAdditionalData(buffer),
...
async function writeWavWithMetadata({
  ...
  while (offset + 8 <= buffer.length) {
    ...
    if (id === EPIC_CHUNK_ID && shouldConsiderEpic) {
      offset = chunkEnd;
      continue;
    }
  ...
  await writeVerified({ sourceAudioPath, outputPath, before: buffer, after: outputBuffer, allowed: changedChunkIdSet });
}
...
async function writeWavStandardMetadata({
  ...
  await writeVerified({ sourceAudioPath, outputPath, before: buffer, after: outputBuffer, allowed: isListInfoRecord });
}
